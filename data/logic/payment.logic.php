...
class paymentLogic {
	 ...
	 * @return array
	 */
	public function getPdOrderSbInfo($pdr_sn, $member_id = null) {
		...
		$order_info = $model_pd->getPdRechargeSbInfo($condition);
		if(empty($order_info)){
			return callback(false,'该订单不存在');
		}

		$order_info['subject'] = '水币充值_'.$order_info['pdr_sn'];
		$order_info['order_type'] = 'vr_order';
		$order_info['pay_sn'] = $order_info['pdr_sn'];
		$order_info['api_pay_amount'] = $order_info['pdr_amount'];
		return callback(true,'',$order_info);
	 ...
	 * 支付成功后修改水币订单状态
	 */
	public function updateVrSbOrder($out_trade_no, $payment_code, $order_info, $trade_no) {
		$post['payment_code'] = $payment_code;
		$post['trade_no'] = $trade_no;
		return Logic('vr_order')->changeSbOrderStatePay($order_info, 'system', $post);
	}
