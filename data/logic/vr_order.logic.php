...
class vr_orderLogic {
	...
	/**
	 * [changeSbOrderStatePay 水币支付订单]
	 * @param  [array] $order_info [description]
	 * @param  [string] $role       [操作角色 buyer、seller、admin、system 分别代表买家、商家、管理员、系统]
	 * @param  [string] $post       [description]
	 * @return [array]             [description]
	 */
	public function changeSbOrderStatePay($order_info, $role, $post){
		try{
			$model_vr_order = Model('vr_order');
			$model_vr_order->beginTransaction();
			$_info = $model_vr_order->getSbOrderInfo(array('pdr_sn'=>$order_info['pdr_sn']));
			
			if (!empty($_info)) {
				//更新订单状态
				$update_order = array();
				$update_order['pdr_payment_time'] = $post['payment_time'] ? strtotime($post['payment_time']) : TIMESTAMP;
				$update_order['pdr_payment_code'] = $post['payment_code'];
				$update_order['pdr_payment_name'] = '扫码支付';
				$update_order['trade_no'] = $post['trade_no'];
