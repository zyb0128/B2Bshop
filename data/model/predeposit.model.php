...
class predepositModel extends Model {
	 ...
	 * @param string $order
	 */
	public function getPdRechargeSbList($condition = array(), $pagesize = '', $fields = '*', $order = '', $limit = '') {
		return $this->table('sb_recharge')->where($condition)->field($fields)->order($order)->limit($limit)->page($pagesize)->select();
	}
	 ...
	 * @param array $data
	 */
	public function addPdRechargeSb($data) {
		return $this->table('sb_recharge')->insert($data);
	}
	 ...
	 * @param string $fields
	 */
	public function getPdRechargeSbInfo($condition = array(), $fields = '*',$lock = false) {
		return $this->table('sb_recharge')->where($condition)->field($fields)->lock($lock)->find();
	}
