...
class rebate_yearModel extends Model {
	public function __construct() {
		parent::__construct('rebate_year');
	}
	 ...
	 * @return bool
	 */
	public function addRebateYear($param){
	 ...
	 * @return bool
	 */
	public function getRebateYear($param){
	 ...
	 * @return bool
	 */
	public function delRebateYear($param){
	 ...
	 * @return array
	 */
	public function getRebateYearList($condition, $field = '*', $page = 0, $order = 'id desc',  $limit = '') {
		$result = $this->field($field)->where($condition)->order($order)->limit($limit)->page($page)->select();
		return $result;
	}

	/**
	 * [getRebateYearListGoodsById 查询列表中的商品信息]
	 * @param  [int] $goodsID [商品ID]
	 * @return [array]          [商品信息]
	 */
	public function getRebateYearListGoodsById($goodsID){
		$result =  Model('goods') -> getGoodsInfoAndPromotionById($goodsID);
		return $result;
	}

	/**
	 * [getRebateYearListMemberById 查询列表中的客户信息]
	 * @param  [type] $memberID [客户ID]
	 * @return [type]           [客户信息]
	 */
	public function getRebateYearListMemberById($memberID){
		$result =  Model('member') -> getMemberInfoByID($memberID);
		return $result;
	}

	/**
	 * [getRebateYearListPrivilegeById 查询列表中的政策信息]
	 * @param  [type] $goodsID [商品ID]
	 * @param  [type] $storeID [店铺ID]
	 * @param  [type] $buyerID [购买者ID]
	 * @return [type]          [description]
	 */
	public function getRebateYearListPrivilegeById($goodsID,$storeID,$buyerID){
