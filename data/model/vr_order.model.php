...
class vr_orderModel extends Model {
	 ...
	 * 取单条水币订单信息
	 */
	public function getSbOrderInfo($condition = array(),$fields = '*') {
		$order_info = $this->table('sb_recharge')->field($fields)->where($condition)->find();
		if (empty($order_info)) {
			return array();
		}
...
//         $order_code[$i-1]['pay_price'] += $diff_amount;
	 ...
	 * @param array $condition
	 */
	public function editSbOrder($data, $condition, $limit = '') {
		return $this->table('sb_recharge')->where($condition)->limit($limit)->update($data);
	}
