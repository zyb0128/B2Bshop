	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if ($order_type == 'r') {
			...
			if (!empty($result['data'])) {
				...
				//水币
				$resultSB = $logic_payment->getPdOrderSbInfo($data['out_trade_no']);
				if (!in_array($resultSB['data']['order_state'],array(ORDER_STATE_NEW,ORDER_STATE_CANCEL))) {
					return true;
				}
				$order_info = $resultSB['data'];
				$resultSB = $logic_payment->updateVrSbOrder($data['out_trade_no'], 'wx_saoma', $order_info, $data["transaction_id"]);
				if (!$resultSB['state']) {
					return false;
				}
				$api_pay_amount = $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount'];
