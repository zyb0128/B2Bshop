...
class predepositControl extends BaseMemberControl {
	...
	public function sb_log_listOp(){
		...
		$model_pd = Model('predeposit');
		$list = $model_pd->getPdRechargeSbList($condition,20,'*','pdr_id desc');


		Tpl::output('list',$list);
		Tpl::output('show_page',$model_pd->showpage());
		self::profile_menu('sb_log_list','sb_log_list');
		Tpl::showpage('predeposit.sb_log_list');
	}

	/**
	 * 水币充值
	 */
	public function recharge_sb_addOp(){
		if (!chksubmit()){
			//信息输出
			self::profile_menu('sb_log_list','sb_log_list');
			Tpl::showpage('predeposit.sb_log_add');
			exit();
		}
		...
		if ($insert) {
			//转向到商城支付页面
			redirect(SHOP_SITE_URL . '/index.php?act=buy&op=pd_sb_pay&pay_sn='.$pay_sn);
		}
	...
	private function profile_menu($menu_type,$menu_key=''){
		$menu_array = array(
			...
			array('menu_key'=>'sb_log_list', 'menu_name'=>'水币明细', 'menu_url'=>'index.php?act=predeposit&op=sb_log_list'),
		);
		switch ($menu_type) {
			case 'rechargeinfo':
